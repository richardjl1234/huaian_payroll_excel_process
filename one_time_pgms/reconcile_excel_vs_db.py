...
# 数值 tolerance：hash 不等但 |diff| <= TOLERANCE 的两行视为匹配。
# 边界用 <=（非 <）：典型例子 14.084999999... 的 Excel 端 hash 出 14.08，
# DB 端因 production 的 pd.to_numeric 截断成 14.085 再 ROUND_HALF_UP 出 14.09，
# 差正好 0.01，必须在容忍范围。
# 用户决定保留 14.09（不修 production pipeline），只在 reconcile 层兼容。
...
def normalize_value(v, col_name=None):
    ...
    - 其他列：
        ...
        * 能转数字的 → Decimal 量化（必须用 ROUND_HALF_UP，与 production 一致；
          用 ROUND_HALF_EVEN 会导致 10.125 → 10.12 vs production 10.125 → 10.13 不匹配）
        * 无法解析的字符串保持原样
    """
...
def _normalize_str_field(v):
    """归一化非数值字段值：None / 'None' / 'null' / 'nan' / '' → ''
    否则 str(v).strip()"""
    if v is None:
        return ''
    if isinstance(v, float) and pd.isna(v):
...
def _tolerance_pair_diff(excel_dict, db_dict):
    """计算两个 row_dict 之间所有数值列的最大绝对差异。
    返回: (max_diff: Decimal|None, non_numeric_match: bool, all_within: bool)
        - non_numeric_match: 非数值字段是否全部相等（None/空串/None字符串视为相等）
        - max_diff: 最大 |excel - db|；None 表示无法比较
        - all_within: non_numeric_match AND 所有数值列 max_diff <= TOLERANCE
    """
    non_numeric_match = True
    for c in EXPECTED_COLUMNS:
        if c in NUMERIC_COLUMNS:
            continue
        e_val = _normalize_str_field(excel_dict.get(c))
        d_val = _normalize_str_field(db_dict.get(c))
        if e_val != d_val:
    ...
    for c in NUMERIC_COLUMNS:
        e_dec = _to_decimal(excel_dict.get(c))
        d_dec = _to_decimal(db_dict.get(c))
        if e_dec is None or d_dec is None:
            return None, non_numeric_match, False
        diff = abs(e_dec - d_dec)
        if diff > max_diff:
            max_diff = diff

    all_within = non_numeric_match and (max_diff <= TOLERANCE)
    return max_diff, non_numeric_match, all_within


def _non_numeric_key(row_dict):
    """非数值字段组成的元组 key，用于 tolerance pass 索引
    None / 'None' / 'null' / 'nan' / '' 一律归一为 ''，避免 DB 的字面 'None' 字符串与
    Excel 端空串对不上导致 tolerance 错失。"""
    parts = []
    for c in EXPECTED_COLUMNS:
        if c in NUMERIC_COLUMNS:
            continue
        parts.append(_normalize_str_field(row_dict.get(c)))
    return tuple(parts)
...
def reconcile_excel_vs_db(excel_data, db_data):
    """对每 (file, sheet) 做集合差集，再做 tolerance 匹配

    两阶段：
    1) hash 集合差集：找 hash 完全不等的行 → 漏行/多行
    2) tolerance 匹配：对剩余 (漏, 多) 对，检查非数值字段相等 AND
       所有数值列 |diff| < 0.01 → 视为匹配，从 missing/extra 中各剔除
       并记录到 samples_tolerance_matched 供审计

    状态：
    - UNREADABLE：损坏文件
    - OK：原 hash 差集 = 0（含 tolerance 全解决的）
    - TOL：有 hash 差集但被 tolerance 全部解决（仍有 tolerance_matched > 0）
    - WARN：有 hash 差集且 tolerance 解决后仍有剩余
    """
    report = []
    ...
    for key in sorted(all_keys):
        ...
        # 2) tolerance pass
        # 按 hash 找回原 row_dict
        excel_hash_to_row = {r["row_hash"]: r["row_dict"] for r in excel_rows}
        db_hash_to_row = {r["row_hash"]: r["row_dict"] for r in db_rows}
        missing_rows = [excel_hash_to_row[h] for h in missing_in_db]
        extra_rows = [db_hash_to_row[h] for h in extra_in_db]

        # 索引 extra_rows by non-numeric key，避免 O(n*m) 扫描
        extra_by_key = defaultdict(list)
        for row in extra_rows:
            extra_by_key[_non_numeric_key(row)].append(row)

        tolerance_matched = []  # [(excel_row, db_row, max_diff), ...]
        unmatched_missing = []  # 剩余漏行
        used_extra_ids = set()  # 已被匹配的 extra row 的 id() 标记

        for excel_row in missing_rows:
            key_tuple = _non_numeric_key(excel_row)
            candidates = extra_by_key.get(key_tuple, [])
            matched = False
            for db_row in candidates:
                if id(db_row) in used_extra_ids:
                    continue
                max_diff, _, all_within = _tolerance_pair_diff(excel_row, db_row)
                if all_within:
                    tolerance_matched.append((excel_row, db_row, max_diff))
                    used_extra_ids.add(id(db_row))
                    matched = True
                    break
            if not matched:
                unmatched_missing.append(excel_row)
        ...
        report.append({
            ...
            "tolerance_matched": len(tolerance_matched),
...
def generate_html_report(reconcile_report, duplicates, audit_findings, load_log_rows, hour_info_rows, db_l19_count, output_path):
    ...
    tol_count = sum(1 for r in reconcile_report if r["status"] == "TOL")
    ...
    total_tol_matched = sum(r.get("tolerance_matched", 0) for r in reconcile_report)
    total_unmatched_missing = sum(r.get("unmatched_missing", 0) for r in reconcile_report)
    total_unmatched_extra = sum(r.get("unmatched_extra", 0) for r in reconcile_report)
    ...
    html = [
        ...
        ".TOL{color:#06c;font-weight:bold;}",
        ...
        f"<li>TOL (tolerance 解决全部差异): <b class='TOL'>{tol_count}</b></li>",
        f"<li>WARN (tolerance 未解决剩余差异): <b class='WARN'>{warn_count}</b></li>",
        f"<li>UNREADABLE (损坏文件): <b class='UNREADABLE'>{unrd_count}</b></li>",
        f"<li>Hash 漏行 (Excel 有 / DB 无): <b>{total_missing}</b></li>",
        f"<li>Hash 多行 (DB 有 / Excel 无): <b>{total_extra}</b></li>",
        f"<li>Tolerance 匹配 (|diff| < 0.01): <b class='TOL'>{total_tol_matched}</b></li>",
        f"<li>剩余真实漏行 (tolerance 后): <b class='WARN'>{total_unmatched_missing}</b></li>",
        f"<li>剩余真实多行 (tolerance 后): <b class='WARN'>{total_unmatched_extra}</b></li>",
        f"<li>跨文件重复 (content_hash 相同): <b>{len(duplicates)}</b></li>",
        f"</ul>",
        f"<p><i>注: TOL 状态表示 hash 不等但 |数值 diff| &lt; 0.01 的行被自动匹配（用户接受 tolerance）"
        f"；WARN 状态表示 tolerance 未能解决、需人工审查的差异。</i></p>",
        f"</div>",
    ]
    ...
    html.append("<table><tr><th>文件</th><th>Sheet</th><th>Excel 行数</th><th>DB 行数</th>"
                "<th>Hash 漏行</th><th>Hash 多行</th><th>Tolerance 匹配</th><th>剩余漏行</th><th>剩余多行</th><th>状态</th></tr>")
    for r in reconcile_report:
        ...
        html.append(
            ...
            f"<td>{r.get('tolerance_matched', 0)}</td>"
            f"<td>{r.get('unmatched_missing', 0)}</td>"
            f"<td>{r.get('unmatched_extra', 0)}</td>"
            f"<td class='{status_class}'>{r['status']}</td></tr>"
        )
    html.append("</table>")

    # Tolerance 匹配明细（被 tolerance 吃掉的行对，让用户能看到实际差异）
    tol_rows = [r for r in reconcile_report if r.get("samples_tolerance_matched")]
    if tol_rows:
        ...
        for r in tol_rows[:30]:
            for excel_row, db_row, max_diff in r["samples_tolerance_matched"]:
                # 找哪些数值列有差异
                diff_fields = []
                for c in NUMERIC_COLUMNS:
                    e_v = excel_row.get(c)
                    d_v = db_row.get(c)
                    try:
                        e_d = _to_decimal(e_v) or Decimal('0')
                        d_d = _to_decimal(d_v) or Decimal('0')
                        if e_d != d_d:
                            diff_fields.append(f"{c}={e_d}↔{d_d}")
                    except Exception:
                        pass
                excel_repr = ", ".join(f"{k}={excel_row.get(k)!r}" for k in ['职员全名', '工序', '客户名称', '型号'] if excel_row.get(k))
                db_repr = ", ".join(f"{k}={db_row.get(k)!r}" for k in ['职员全名', '工序', '客户名称', '型号'] if db_row.get(k))
                html.append(
                    f"<tr><td>{r['file_name']} :: {r['sheet_name']}</td>"
                    f"<td>{excel_repr}<br><b>差异: {', '.join(diff_fields)}</b></td>"
                    f"<td>{db_repr}<br>&nbsp;</td>"
                    f"<td>{max_diff}</td></tr>"
                )
        if len(tol_rows) > 30:
            html.append(f"<tr><td colspan=4>... 还有 {len(tol_rows) - 30} 个文件有 tolerance 匹配 ...</td></tr>")
        html.append("</table>")
...
def main():
    ...
    print("\n--- 阶段 3: 集合差集 + tolerance 匹配 ---")
    reconcile_report = reconcile_excel_vs_db(excel_data, db_data)
    ok_count = sum(1 for r in reconcile_report if r["status"] == "OK")
    tol_count = sum(1 for r in reconcile_report if r["status"] == "TOL")
    warn_count = sum(1 for r in reconcile_report if r["status"] == "WARN")
    total_tol = sum(r.get("tolerance_matched", 0) for r in reconcile_report)
    print(f"  OK: {ok_count}, TOL: {tol_count} (匹配 {total_tol} 行), WARN: {warn_count}")
