...
### batch_process.py 执行流程图

```mermaid
flowchart TD
    Start([🔍 程序启动]) --> CheckArgs{检查命令行参数?}
    
    CheckArgs -->|无参数| BatchMode[📦 批处理模式<br/>batch_process_main]
    CheckArgs -->|有参数| SingleMode[📄 单文件模式<br/>process_single_file]
    
    BatchMode --> GetFiles[get_excel_files<br/>获取所有Excel文件]
    SingleMode --> ValidateFile{验证文件存在?}
    
    ValidateFile -->|不存在| FileError[✗ FileNotFoundError]
    FileError --> End1([❌ 结束])
    
    ValidateFile -->|存在| SingleFiles[设置单文件列表]
    
    GetFiles --> ProcessFiles[_process_excel_files<br/>处理Excel文件]
    SingleFiles --> ProcessFiles
    
    ProcessFiles --> CleanDB{clean_db?}
    
    CleanDB -->|是| CleanTables[clean_database_tables<br/>清空数据库表]
    CleanDB -->|否| SkipClean[跳过清理]
    
    CleanTables --> IterSheets[迭代 sheet_gen<br/>生成 SheetContents]
    SkipClean --> IterSheets
    
    IterSheets --> ForEachSheet{遍历每个工作表}
    ForEachSheet -->|还有| LogSheet[📋 记录: 处理中...]
    LogSheet --> IterDFs[迭代 df_gen<br/>生成 SplitDataFrame]
    
    IterDFs --> ForEachDF{遍历每个数据框}
    ForEachDF -->|还有| LoadDB[load_df_to_db<br/>加载到数据库]
    LoadDB --> CheckResult{加载成功?}
    CheckResult -->|是| SuccessLog[✓ 记录成功日志]
    CheckResult -->|否| FailLog[✗ 记录失败日志]
    
    SuccessLog --> NextDF
    FailLog --> NextDF
    NextDF[下一个SplitDataFrame] --> ForEachDF
    
    ForEachDF -->|遍历完成| NextSheet[下一个工作表]
    NextSheet --> ForEachSheet
    
    ForEachSheet -->|遍历完成| ReturnStats[返回处理统计]
    
    ReturnStats --> EndBatch[batch_process_main<br/>记录完成日志]
    EndBatch --> End2([✅ 结束])
    
    ForEachDF -->|遍历完成| ReturnStats2[返回处理统计]
    ReturnStats2 --> EndSingle[process_single_file<br/>记录完成日志]
    EndSingle --> End3([✅ 结束])

    style FileError fill:#ff6b6b,color:#fff
    style SuccessLog fill:#51cf66,color:#fff
    style FailLog fill:#ff6b6b,color:#fff
```
...
```mermaid
sequenceDiagram
    participant BP as batch_process.py
    participant SG as sheet_gen.py
    participant DF as df_gen.py
    participant SP as sheet_processor.py
    participant DB as SQLite DB
    
    Note over BP: 程序入口
    BP->>SG: get_excel_files()
    SG-->>BP: excel_files列表
    
    BP->>SG: sheet_gen(excel_files)
    loop 对每个Excel文件
        SG-->>BP: SheetContents对象
        BP->>DF: df_gen(SheetContents)
        loop 对每个分割数据框
            DF-->>BP: SplitDataFrame对象
            BP->>SP: load_df_to_db(split_df, ...)
            SP->>DB: INSERT INTO payroll_details
            SP->>DB: INSERT INTO load_log
            DB-->>SP: 执行结果
            SP-->>BP: 成功/失败消息
        end
    end
